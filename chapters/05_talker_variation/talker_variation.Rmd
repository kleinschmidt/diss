...
Given the well-known dialect-specific variation in vowel production, it's somewhat surprising that dialect-level cue distributions do not, in our models, provide a substantial overall improvement in correct recognition.  However, dialects themselves vary in how much they deviate from norms of Standard American English (Figure @fig:vowel-kl-by-dialect). There is corresponding variability in the utility of knowing a talker's dialect, depending on what that dialect is (Figure @fig:overall-acc-by-dialect). Knowing a dialect 

shows, dialect increases the probability of correct recognition only for talkers from the Mid-Atlantic and North regions.

To address this possibility, we examine how using dialect-level formant distributions affects the probability of making particular _errors_ that would result from ignoring dialect-specific shifts. For instance, in the Northern Citiies Shift, the `ae` vowel is raised, resulting in a distribution very much like the standard `eh` distribution. If dialect is ignored, then `ae` tokens from talkers from a dialect region with this shift will be mistaken for `eh`.

```{r overall-acc-by-dialect, fig.width=10, fig.height=5, fig.cap='The utility of socio-indexical variables varies across dialects. Dialect itself is particularly informative only for talkers from the Mid-Atlantic and North regions.'}
...
useful, more informative, and more easily inferred for vowel than for stop
...
* Summary of results
    * on the whole, socio-indexical variables are _informative_ about linguistic
      cue distribution, _useful_ for improving speech recognition, and
      themselves can be inferred on the basis of acoustic-phonetic cues alone.
    * these variables differ in utility and informativity, both within a single
      phonetic contrast/set of cues, and between different contrasts/cues.
    * on the whole, informativity and utility mirror each other. However,
      utility for speech recognition and socio-indexical judgements do not
      always mirror each other.
* What of our goals?
    1. _Informativity_: socio-indexical variables are more informative for
       vowels than for stop voicing. And talker identity is by far the most
       informative of all variables considered about both vowel and stop
       voicing. After that, other grouping variables are uniformly uninformative
       for stop voicing, and for vowels, talker sex is next, followed by
       dialect.
    2. _Utility_
    3. _Inferability_
* Points to discuss
    * These results help us make more specific predictions about what
      distributions listeners should (according to the ideal adapter) be
      tracking.
    * Help explain discrepancies in how listeners generalize experience from
      one talker to another, for different cues/contrasts.
    * More basically: show that we can think of socio-linguistic judgements as
      an inference process, where listeners use their knowledge of the
      distribution of variants produced by different groups to infer how likely
      it is that a particular talker belongs to a group.



* normalization vs. adaptation.
    * both are well supported as strategies that listeners use for dealing with
      variability [@WeatherholtzInPress]
    * ...normalization changes usefulness of some variables. dialect is
      basically useless without normalization.
...
<!-- missing the connection: listeners can benefit from _structure_ in
variability over talkers/situations. we don't know what that structure is, at
least in a way that allows for making more than qualitative predictions -->
...
specific estimates and higher classification accuracy
[Figures @fig:vowel-vot-kl-plot and @fig:overall-accuracy-group-known; @Hillenbrand1995; @Feldman2013a].


<!-- When adapting to an unfamiliar talker, listeners face a tradeoff.  On the
one hand, a talker's own productions are the most informative about how that
talker will realize their linguistic intentions in the future. On the other
hand, listeners often have very few tokens from an unfamiliar talker themselves,
and estimates of an underlying distribution from a small number of observations
are either highly uncertain or noisy and more likely to be wrong. Thus, in cases
where cue distributions are similar across talkers, listeners can benefit by
treating those observations as if they came from the same underlying
distribution. But in cases where talkers differ a lot, lumping observations from
different talkers or groups together will lead to reduced comprehension accuracy
and/or slowed adaptation to a talker's own cue distributions. The analyses
reported here show that _both_ types of situations actually occur in variation
across talkers. -->
...
* Generative model: good for more than just correct/incorrect recognition
  accuracy. not always 100% certain, lots of uncertainty/overlap in normal
  speech, need to have a good model of cue distributions in order to deal with
  that uncertainty effectively.
    * (( evidence that people are sensitive to the distributions? McMurray,
      Clayards, ))
    * (( what does this have to do with what we've shown here...is there
      anywhere where these measures make opposite predictions? and where would
      that really matter? uncertainty maintenance is one place))

## Sociolinguistic stuff

* Socio-linguistic judgements can also be thought of as inference.
* simultaneously making all kinds of inferences: 
    * what a talker is saying
    * how a talker says things
    * _who_ a talker is, relative to other talkers. important both for speech
      perception, but also important in and of itself.
* the important point here is that this way of thinking about speech perception
  situates social inferences as a central aspect of speech processing. in fact,
  in this view social inferences may well be inextricable from linguistic
  inferences.
* Important caveat: this is not, per se, a complete model of socio-linguistic
  judgements. the inferences that a lisener would draw in this view depends on
  the generative model of social variables that listeners have. one way to think
  about this is that inference depends not only on the cue distributions but on
  the prior over social variables themselves. 
    * These need careful sociolinguistic work to tease out. A good example of
      this work is @Levon2014. If listeners believe that a working class
      identity and homosexuality identity cannot occur in the same person, then
      their inferences about whether or not someone is a gay man will depend on
      whether or not they believe that person to be working class
      [@Levon2014 refers to these as a listener's stereotypes or ideologies]
* (( PROBABLY TOO MUCH FOR THIS PAPER )) if this is true, what does this
  perspective contribute above and beyond "traditional" socioliguistics?
    * (quantifiable) notions of cue reliability. WHY some judgements block
      others, and which.
    * for instance, if cues to gayness fall closer to normal range of variation
      may be easier to explain away than cues to class; prior belief that you
      can't be both gay and working class blocks the normal inference about
      gayness.

(( something about relevance to sociolinguistics...what do people have knowledge
of? what sorts of inferences should they be able to make? what cues are actually
informative? ))
...
* __take home point__: inference under uncertainty/ideal adapter provides a
  unified approach to speech perception. a set of _conceptual tools_ to address
  both socio- and psycholinguistically interesting questions. and novel
  combinations that are of interest to both.
