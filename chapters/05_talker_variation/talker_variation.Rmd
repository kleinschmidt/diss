...
```{r check-classification, cache=TRUE, dependson=c('vowel-classification', 'vot-classification')}
...
acc_by_vowel <- accuracy %>%
  ...
  group_by(cues, grouping, Vowel, Talker) %>%
  summarise_each(funs(mean), accuracy, accuracy_p)
...
ggplot(acc_by_vowel_summary, aes(x=Vowel, y=accuracy, color=grouping)) +
