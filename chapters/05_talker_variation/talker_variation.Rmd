...
<!-- something about comparing across contrasts/categories. how can you compare level of variability _across_ different cues/categories?? -->
In this paper we present three studies, which focus on three interrelated ways of quantifying this relationship:
...
#### Goal 1: quantify _informativity_ of socio-indexical variables about cue distributions

On the one hand, we know that talker variability is ubiquitous. On the other
hand, qualitatively, different phonetic contrasts exhibit different degrees and
patterns of talker variability in American English. For instance, there is
substantial variability in vowel formant frequencies across talkers, at least
part of which is due to systematic differences due to gender
[@Peterson1952; @Hillenbrand1995] and regional dialect [@Clopper2005]. Voicing
contrasts (e.g., /b/ vs. /p/) are thought to exhibit some talker variability,
but relatively little, and not systematically linked to dialect, sex, or other socio-indexical variables
[@Allen2003; @Lisker1964; @Chodroff2015].[^bilingual-vot]
...
The different role that socio-indexical variables play across cues/contrasts,
however, have not been quantitatively assessed, in part due to the difficulty in
comparing variability _across_ very different cues and category structures.
Thus, one goal of this paper is to introduce a way of assessing these
differences that allows for comparison across contrasts. We use this measure
to establish an approximation of upper and lower bounds on the level of talker
variability, by focusing on contrasts and cues that are commonly considered to
exhibit high and low degrees of variability (vowel formants and stop consonant
voice onset time, respectively).

#### Goal 2: quantify _utility_ of socio-indexical variables for speech recognition

We also know that listeners are sensitive to the fact that talker from different
socio-indexical groups produce phonetic categories in systematically different
ways, and that they use this to guide their speech recognition. Perception of
vowels (and fricatives) is affected by the perceived gender of the talker, which
can be cued by voice quality, visual presentation of a male or female face, or
even explicit instruction [@Strand1999;@Johnson1999;@Strand1996].  Listeners'
perception of vowels also depends on social variables: @Niedzielski1999 found
that if listeners believe that a talker is Canadian, they hear more Canadian
raising than if they believe the talker is American. @Hay2010 found a similar
...
to inform speech perception, but they don't tell us exactly _how useful_ these
variables are. That is, what would be the consequences of _ignoring_ these
variables? How much do these consequences depend on the particular cues,
category, or socio-indexical variable?  Our second study addresses this
question.

#### Goal 3: quanitfy _inferrability_ of socio-indexical variables given cue distributions
...
Again, what we don't know is what information listeners are using to make these
inferences, and even how much information is available to them
[for a review, see @Thomas2002]. Thus our final study present a method of
quantifying how informative different phonetic contrasts and cues are about a
range of socio-indexical variables.
...
stuffed animal that cued New Zealand or Australia.  
