...
<!-- TODO: awkard, revise -->
When thinking about speech perception as an inference process, knowledge of
between-talker variability in cue distributions is helpful, because effective
inference depends on knowing the underlying cue distributions for each
category. But this way of thinking also predicts that some kinds of variability
should affect speech perception itself more than others. For instance, if one
...
That is, the _utility_ of a socio-indexical variable for guiding speech
recognition is not necessarily the same as its _informativity_ about the
underlying cue distributions. Informativity provides an upper bound on utility,
since conditional cue distributions can only improve recognition to the extent
...
variability in cue distributions with the utility of those variables.  It also
provides a framework for making specific, testable predictions about how
listeners actually take advantage of structure in talker variability, both in
using socio-indexical variables to guide speech perception directly and in
learning and structuring knowledge about how cue distributions vary across
talkers.

<!-- TODO: Something about normalization migth be appropriate here: utility can depend on the input representation -->
...
etc. [cf. @Eckert1989; @Eckert2012a; @Labov1972].
...
[^dynamic-socio]: There are arguably other, more dynamic non-linguistic aspects
    that are coded in the speech signal, such as mood <!-- (REF) --> or attitude
    towards our interlocutors <!-- (REF) -->. The approach laid out here---at
...
fact make these sorts of socio-indexical inferences on the basis of speech
alone.  For instance, listeners can classify talker's regional dialect at
above-chance accuracy based on a short except of their speech
...
However, the extent to which such knowledge would actually be useful in
practice---and how informative a particular category's or contrast's cue
distributions are---depends on the actual distribution of cues within and
between groups.
