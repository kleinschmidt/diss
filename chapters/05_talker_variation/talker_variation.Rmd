...
grouping_levels <- c('Marginal', 'Age', 'Sex', 'Dialect', 'Dialect_Sex', 'Talker')
...
vowel_marginal_class <-
  vowel_joint_class %>%
  group_by(Talker, Vowel, Token, category_model, add=TRUE) %>%
...
vot_marginal_class <-
  vot_joint_class %>%
  group_by(Talker, voicing, Token, category_model, add=TRUE) %>%
  marginalize_log('log_posterior') %>%
  normalize_log_probability('log_posterior') %>%
  mutate(group_is = 'Inferred')
...
vowel_accuracy <-
  vowel_class %>%
  ## filter(Vowel == category_model) %>%
  ## mutate(accuracy = posterior_choice,
  ##        accuracy_p = posterior)
  filter(posterior_choice) %>%
  mutate(accuracy = Vowel == category_model)

vot_accuracy <-
  vot_class %>%
  ## filter(voicing == category_model) %>%
  ## mutate(accuracy = posterior_choice,
  ##        accuracy_p = posterior)
  filter(posterior_choice) %>%
  mutate(accuracy = voicing == category_model)

accuracy <- 
  vowel_accuracy %>%
  select(-Age) %>%
  filter(subsample_size == 3 | grouping == 'Talker') %>%
...
accuracy_summary %>%
  filter(group_is == 'Known' | grouping == 'Talker') %>%
...
Also paralleling the cue distributions themselves, using sex-specific distributions provides the biggest boost in recognition for un-normalized formant accuracy. For normalized input, none of the socio-indexical grouping factors provide much of an advantage over the marginal distributions

* for voicing: 
    * slight advantage of talker. others are indistinguishable

```{r by-vowel-acc-group-known}
...
vowel_class_competitors %>%
  filter(group_is == 'Known' | grouping == 'Marginal',
         subsample_size == 7 | grouping == 'Dialect',
         category_model == Competitor) %>%
  ...
  ggplot(aes(x=pair, y=confusions, color=grouping, fill=grouping)) +
  ...
  ggtitle('Socio-linguistically predicted confusions (7 talker subsample, group known)') +
...
Figures {@fig:overall-accuracy-group-known} and {@fig:by-vowel-acc-group-known} suggest that knowing a talker's dialect does not reliably improve the ability to correctly recognize vowels. However, it's hard to judge the talker-by-talker advantage of knowing dialect when uncertainty over talkers is plotted independently for the various grouping factors. Figure {@fig:dialect-over-marginal-sex} clarifies this relationship by plotting the average (and 95% bootstrapped CIs) increase in correct recognition of each vowel for knowing each talkers dialect. Two different baselines are used: the marginal cue distributions, and the sex-specific distributions (compared with dialect+sex).  This suggests that there may be weak effects for `ey`, `ae`, `eh`, and `uh`, but it is hard to draw firm conclusions for two reasons. First, correct recognition is at ceiling for many talkers/vowels. Second, the training data subsampling procedure that is necessary to even allow for comparison across different levels of grouping introduces additional variance that's not directly accounted for here (even though it is indirectly accounted for by bootstrapping over talkers, each of whose models was trained on a different random subsample of talkers).
