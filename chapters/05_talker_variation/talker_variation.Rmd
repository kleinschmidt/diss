...
## 1. split data into LOO train/test sets, restricted to same group
## 2. generate NNN random subsamplings of talkers in training group
## 3. for each subsampling, train models, classify test data, and compute acc

#' @param d data frame with columns for groups, category, and holdout
#' @param groups
train_test_same_group <- function(d, groups, category, holdout, ...) {
  assert_that(has_name(d, groups))
...
x<-  d %>%
  ...
  train_test <- function(data_train, data_test) {
    data_train %>%
      train() %>%
...
vowel_models <-
  ...
  mutate(train_test = map2(data, grouping,
                           train_test_same_group,
