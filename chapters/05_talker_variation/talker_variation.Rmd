...
VOT). Focusing on information theoretic properties of the cue distributions
provides a principled common currency for comparing different socio-indexical
variables, phonetic categories, and cues.
<!-- [^bilingual-vot] -->
...
Knowledge of between-talker variability in cue distributions is, in general,
helpful for robust speech perception. But some kinds of variability affect
speech perception itself more than others. For instance, if one category varies
across talkers in a way that never causes it to overlap with another category,
listeners can safely ignore talker-level variation in that category without
increasing the chance of errors in speech recognition. On the other hand, when a
category varies across talkers or groups in a way that _does_ cause it overlap
with other categories, ignoring talker-conditioned distributions would, in fact,
lead to slowed or inaccurate speech recognition. The same is true of broader
socio-indexical groupings, like sex or dialect.

That is, the _utility_ of a socio-indexical variable for improving speech
recognition is not necessarily the same as the _informativity_ of that variable
about the underlying cue distributions. Informativity provides an upper bound on
utility, since conditional cue distributions can only improve recognition to the
extent that they are actually different from marginal (un-conditioned)
distributions. This raises the possibility that two equally informative
socio-indexical variables may not be equally useful for the purposes of speech
perception.  It is possible that listeners would not _track_ cue distributions
conditioned on informative socio-indexical variables which are not useful for
speech perception.
...
from familiar talkers than unfamiliar talkers [@Nygaard1998].
...
cue distributions and phonetic categorization.  
