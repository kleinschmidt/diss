...
# What do you expect from an unfamiliar talker? Infering listeners' priors beliefs {#chap:infer-priors}
...
and do not report the variance.
<!-- TODO: other kronrod points. how different from our model. prevoicing -->
...
<!-- TODO: footnote on exemplar approaches being broadly compatible? -->
<!-- TODO: what kind of models would NOT predcit this? to avoid "impression of confirmatory modeling" -->
...
<!-- TODO: what did kronrod et al. do? -->
...
\theta = \{ \mu_\mathrm{b}, \sigma^2_\mathrm{b}, \mu_\mathrm{p},
            \sigma^2_\mathrm{p} \}
$$ 
where $\mu_c$ is the mean VOT of category $c$ and $\sigma^2_c$ is its variance. We do not include the mixing weight (category prior probability) as a parameter, assuming that each category is equally likely. This is true in the context of our experiment, and moreover the only effect of unequal weights is a constant shift in the category boundary, regardless of the distributions [@Feldman2009a].  As in
@Kleinschmidt2015, we use an independent, conjugate Normal-$\chi^{-2}$ prior for
each category, with parameters $\phi$ [@Gelman2003]
...
in these prior expectations. In this parametrization, the prior confidence parameters $\kappa_0$ and $\nu_0$ can be interpreted as _pseudo-counts_, or the number of hypothetical observations that listeners' prior beliefs about the category means and variances are based on.  Note that, as in
our previous modeling work, these prior confidence parameters are
shared between the two categories. That is, we assume that listeners have equal confidence in their prior beliefs about the mean of /b/ and of /p/ (and likewise for the variance of /b/ and /p/).[^unequal-conf]
...
[^trial-order]: The order of trials _does_ matter, in general
    ...
    previous modeling work is attempts to model such trial effects. However, the
...
dataset on the table, since we don't consider listeners' own judgements about
...
back and re-evaluate previous judgements. This is not _necessarily_ an
...
discrimination data, @Kronrod2012; as well as from production by @Goldrick2013]. The
inferred prior beliefs are in reasonably good agreement with this typical
talker, with one exception: 
...
between the two types of /b/ distributions that talkers actually produce.  As
Figure @fig:inferred-prior shows, the inferred /b/ distribution is an
approximate compromise between the low-variance, short-lag VOT and the
high-variance, prevoiced VOT distributions observed by @Goldrick2013.

This possibility suggests two directions for future work. First, large-scale corpus
studies of VOT distributions are needed to determine to what extent the
distribution of /b/ mean VOTs is really bimodal across talkers. However, the
only study of this type that we are aware of considers only non-negative VOTs
@Chodroff2015 and thus excludes talkers who pre-voice their /b/.  Second, more
modeling work is needed to test whether a multimodal prior is justified given
the adaptation data, and if so, whether it would change the inference about
listeners' prior expectations for /b/.
...
prior_stats_variability <-
  ...
  summarise_each(funs(mean, var, sd), mu, sigma2, sigma)
...
Lastly, we can ask whether the model-inferred beliefs match the extent to which talkers actually vary in the VOT distributions the produce.

Qualitatively, the prior parameters show that 
listeners have 
high confidence in their prior beliefs, with confidence about category
variances ($E(\nu_0) = `r prior_expected['nu_0']`$) higher than confidence
about  means ($E(\kappa_0) = `r prior_expected['kappa_0']`$).  Both
...
In this model, these confidence parameters also in large part control how much
...
expected _within-category_ variance divided by the mean confidence pseudocount,
$\sigma_0^2 / \kappa_0$ (and the cross-talker standard deviation is hence $\sigma_0 / \sqrt{\kappa_0}$).
...
By one estimate, actual cross-talker variability is on the
order of a standard deviation of 3ms VOT for /b/ [short-lag; @Goldrick2013] and
15ms VOT for /p/ [@Baese-Berk2009].
...
modeling work to explore the feasibility and consequenes of relaxing this
assumption.
...
experiment, we go one step further, shifting the means of the /b/ and /p/
...
test of the predictive utility of this framework.
...
<!-- inferred beliefs, as well as testing --> test how much the uncertainty
...
post-test, see below), after which `r n_clean2 %$% sum(n)` subjects remained for
analysis (no subjects had previously participated in one of our VOT studies). Subjects were
evently distributed across conditions (range of `r n_clean2 %$% min(n)` to 
...
the following post-test trials _within_ the same subjects. Figure
...
@fig:boundary-violin-plots-exp2, un-shaded distribution). It finally allows
testing out-of-sample, same condition prediction accuracy for the
belief-updating model results (Figure @fig:plot-exp2-predictions-first-third).
...
predicted classification functions (95% posterior predcitive intervals) for the
...
`r mod_pred_gof_exp2_by_third %>% filter(third==1) %$% LL_ratio`). In particular,
the predicted classification function fits very well for the 10ms /b/, 50ms /p/
mean condition. This condition is the same as one condition from Experiment 1,
so this shows that the within-condition, out-of-sample prediction accuracy is
good, despite the different post-test procedure.
...
(Figure @fig:model-fit-classification), which the model was trained on. This
uncertainty, however, is revealed by input distributions that are more diverse, like those
...
The results of this experiment provide further evidence that VOT distributional
learning is constrained by listeners' expectations. Using more extreme
...
While listeners' failure to adapt might seem to indicate a failure of
distributional learning. However, computational modeling reveals that these
apparent differences between Experiments 1 and 2 are not only _consistent_ with
belief updating starting from a singel set of prior beliefs, but are actually
_predicted_ based on the adaptation in Experiment 1.
...
typical talker sounds like and how much talkers vary. Moreover, the expectations that
we infer listeners to have for an unfamiliar talker match the actual cue
...
(( TODO: stuff on prior confidence vs. other modeling work here? ))

<!-- TODO: explained by physical constraints on VOTs? no, the VOTs we use are well within the range that naturally occur. the difference is the STATISTICS -->
...
boundaries occur at a small number of VOT values. Moreover, non-human animals
also categorize voiced and voiceless stops along these boundaries as well
[@Kuhl1975]. Moreover, the typical boundary between (short-lag) voiced and voiceless word
initial stops is approximately the same as the temopral asynchrony required for
neural responses to separately time lock to two events [@Steinschneider1994].

Another possibility is that listeners' expectations are driven by
...
constrained) by their experience with _within-group_ variabiltiy, not only population-level variability. For instance, if listeners pick up on the fact that
...
[see, e.g., @Niedzielski1999; @Levon2014].
...
form, the Normal-$\chi^{-2}$ distribution. This is mathemtically convenient, but
...
### How much confidence do listeners have in their prior beliefs?
...
that are updated based on experience with the exposure talker. Moreover, these
inferred beliefs effectively predict adaptation to different distributions.
