...
separation between /b/ and /p/ mean VOT of 40ms.
...
($n=`r n_excl2 %$% sum(n)`$) 
who failed to classify words with unambiguous VOTs reliably, after which 
...
of `r sepmeans_test %>% filter(subject==first(subject)) %>% nrow()` additional
trials with VOTs evenly distributed from -10 to 50ms. This additional phase was
necessary because of the large separtion between /b/ and /p/ clusters in the
exposure distributions meant that in some conditions there were no trials with
VOTs anywhere near the predicted (or typical) boundaries.
...
## generate and plot predictions AT TRIAL 0 (beginning of test phase)
exp2_predict_at <- data_frame(trial = 0,
