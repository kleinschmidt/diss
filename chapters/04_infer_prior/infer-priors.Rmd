...
f0, and /b/ to low values of both cues. In one condition, listeners were exposed to a talker who produced
a positively correlated distribution of these cues. These listeners adapted to this distribution, and used
f0 to categorize stops with ambiguous VOTs. In another condition, heard a talker
who produced an *un*correlated distribution, where f0 is uninformative. Listeners in this condition also adapted, learning to ignore f0 even for ambiguous
VOTs. But when exposed in a third condition to a talker who produced an *anti*correlated
distribution, listeners did _not_ adapt.  Despite the fact
...
vot_range <- c(-20, 50)
vot_to_hcl <- function(vot) hcl(h = (vot-vot_range[1])/diff(vot_range)*330 - 15,
                                c = 100, l = 65)
colors_exp1 <- conditions_exp1 %>% mutate(color = vot_to_hcl(ideal_boundary))

scale_color_exp1 <- scale_color_manual('/b/, /p/\nmean VOT',
                                       breaks = colors_exp1$vot_cond,
                                       values = colors_exp1$color)
...
Reliable classification was defined by accuracy at 0 and
70ms VOT, as extrapolated via a logistic GLM, of at least 80% correct.
...
Our distributional learning procedure is based on @Clayards2008. Figure @fig:beach-peach shows an example trial display. On each trial,
...
classification function (fit by a logistic GLM) to the classification
...
this eliminates the possibility that listeners could go back and re-evaluate
their previous judgements, which is anathema to a Bayesian belief updating
model.
...
colors_exp2 <- sepmeans_conds %>% mutate(color = vot_to_hcl(ideal_boundary))
scale_color_exp2 <- scale_color_manual('/b/, /p/\nmean VOT',
                                       breaks = colors_exp2$vot_cond,
                                       values = colors_exp2$color)
