...
```{r inferred-prior, fig.width=4, fig.height=2, fig.cap="Expected cue distributions based on the prior beliefs inferred here from behavioral adaptation data (solid lines). Plotted with VOT distributions measured by @Kronrod2012 based on a combination of classification and discrimination behavior, and from production data by @Baese-Berk2009 (for /p/) and @Goldrick2013 (/b/)."}
...
if they produce mostly prevoiced. @Goldrick2013 found that, across
`r nrow(prop_pre_by_talker)` 
talkers, the highest percentage of prevoiced word-initial stops was 
`r prop_pre_by_talker %$% round(prop_pre*100) %>% max()`%. 
...
* further evidence that listeners can adapt to unusual accents distributional
  learning. goes beyond prior work by showing that distributional learning is
  _constrained_
* where do constraints come from? 
    * one idea: prior experience with other talkers. we've showed that the
      constraints we observed are at least _consistent_ with this possibility
        * evidence for the importance of prior beliefs that are informed by the
          structure of the world in other domains too. [@Anderson1991 and from
          conclusion...]
    * other possible sources of constraints (self-production, innate auditory)
    * testing with other contrasts/cues will help resolve this. 
    * (for instance: next chapter shows that talker variability for VOT is
      qualitatively and quantitatively different than for vowels. vowels are
      particularly interested because there's both stylistic variation that has
      to be learned, and physiological variation that can be normalized using
      basic auditory processes)
* you can infer prior beliefs, which are similar to actual patterns of
  cross-talker variability and which predict adaptation to other distributions
    * useful tool: probe listeners' subjective expectations (which may well be
      different from variation in the world, and require perception data that is
      somewhat easier to collect and analyze than production data)
    * there are limits on the usefulness of this tool though. the prior beliefs you
      infer are fundamentally bound by the model you use. you can still have
      predictive accuracy with a model that does not even match the structure of the
      data, to say nothing of what people are actually doing.
