...
At first blush, this conflicts with our previous modeling work on another phonetic contrast,
/b/-/d/, which found confidence values that were one or two orders of magnitude
smaller than those inferred here [@Kleinschmidt2015].  This
...
votcorpora::vot %>%
  filter(place == 'lab') %>%
  group_by(prevoiced, voicing, phoneme, subject) %>%
  summarise(mean_vot = mean(vot)) %>%
