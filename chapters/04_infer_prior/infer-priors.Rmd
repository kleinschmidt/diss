...
The particular set of distributions we chose was informed by the particular way that the model fit the data. In particular the model's _uncertainty_ about the joint combination of category means and variances given the data was telling: <!-- MEH. we don't really address this by updating the modeling beliefs, right? so why bother. -->

Another, secondary goal is to 

* Why?
    * There's lots of other possible distributions. varied only one thing: the
      overall mean VOT. Go one step beyond that, varying the distance between
      distributions as well.
    * In particular, how do people handle negative VOTs? Suggested that high
      variance of /b/ is a compromise that reflects mixture of short-lag and
      prevoiced /b/ distributions. if people's judgements are driven by both of
      those, then shifting the /b/ cluster way down shouldn't have much of an
      effect, since the short-lag distribution will be unaffected. <!-- needs
      more explanation here -->
    * test predictive power of model: can it predict adaptation to distributions
      not trained on?
