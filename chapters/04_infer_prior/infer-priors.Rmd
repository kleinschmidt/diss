...
[^exemplar]: It is not immediately clear whether these results are compatible
    ...
    unsupervised distributional learning with exemplar models. Moreover, it is
    not clear that an exemplar model would be able to extract talker-specific
    distributional regularities in the absence of any labeling information,
    since the initial exemplars will be categorized based on those stored from
    previous talkers, and absent any mechanism to go back and revised these
    early categorizations the model may become dug in to the clusters
    established by previous talkers. Whether or not these results are compatible
    with exemplar theories is thus best left to future work using an implemented
    model.
...
That is, Experiment 2 is aimed at allowing future modeling work that can provide
...
distribution representations, this has the potential to go beyond the
